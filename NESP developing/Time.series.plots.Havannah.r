...
library(data.table)
library(openair)
data.dir="C:/Users/jc246980/Documents/Current projects/Additional work/NESP analysis/Havannah/"; setwd(data.dir)
image.dir="C:/Users/jc246980/Documents/Current projects/Additional work/NESP analysis/Havannah/"

####Dunk Island


tdata=data.frame(read.csv("Havannah.csv"))
flowdata=data.frame(read.csv("Burdekin discharge.csv"))

trapdata=data.frame(read.csv("Havannah trap accumulation.csv"))
...
p3.trap <- ggplot2::ggplot(data = trapdata, aes(colour = Trap, x = trapdata$Date.In, xend = trapdata$Date.Out, y = trapdata$Value, yend = trapdata$Value)) +theme_classic(base_size = 12)+
    geom_segment(lwd=1.5)+ theme(legend.position = c(0.95,0.2))+
	theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),axis.title.y = element_text(colour="grey20",size=12))+ylab(bquote('Trap acc.' ~(mg.cm^2~d^-1)))+
		scale_color_manual(values=c("chocolate4","chocolate2", "chocolate"))+scale_y_continuous(limits = c(0, 15),position = "left")+
		scale_x_date(expand=c(0,0))+annotate(geom="text", x=as.Date("2016-06-25"), y=ypos, label= "a.", size=6) 
...
png(paste(image.dir,"Havannah September02.png",sep=''),width=40, height=22, units='cm', res=500, pointsize=10, bg='white')
