...
data.dir="C:/Users/jc246980/Documents/Current projects/NESP/RMS calculations/Havannah/"; setwd(data.dir)
RMScurrent<-fread("Havannah_RMS_ALL.csv",sep=",")
...
MG2=MG2[which(MG2$deploys %in% c(1,2,3,4,5,10,12)),] # remove deployments with current meter data issues
...
png(paste(image.dir,"Havannah Island July 2020.png",sep=''),width=40, height=(40/3)*2, units='cm', res=500, pointsize=10, bg='white')
...
p.SSC <- ggplot(MG2,aes(DateTime, SSC)) + geom_point(stat="identity", colour="#FF7F00", size=0.3) + theme_classic(base_size = 12)+scale_y_continuous(position = "left")+
        ...
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),axis.title.y = element_text(colour="grey20",size=12))+ylab(bquote('SSC' ~(mg.L^-1)))+
		scale_x_datetime(expand=c(0,0),limits =xpos)+annotate(geom="text", x=as.POSIXct("2016-06-05 12:00", format = "%Y-%m-%d %H:%M"), y=40, label= "a.", size=6)+ylim(0,50)
...
p.RMScurrent <-ggplot(MG2,aes(DateTime, as.numeric(currentRMS))) + geom_point(stat="identity", colour="darkslateblue", size=0.3) + theme_classic(base_size = 12)+
		scale_y_continuous(position = "right",limits=c(0,0.0025))+
		theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),axis.title.y = element_text(colour="grey20",size=12))+ylab(bquote('RMS (current)'))+
...
png(paste(image.dir,"Havannah 2016 June-Dec.png",sep=''),width=40, height=(40/3)*2, units='cm', res=500, pointsize=10, bg='white')
        par(mar=c(4,4,1,1),cex=1,oma=c(2,0,1,0.5)) #		 
