...
deploys=deploys[which(deploys$Site=="Orchard"),]
tdata$deploys<-NA
...
data.dir="C:/Users/jc246980/Documents/Current projects/NESP/RMS calculations/Orchard/"; setwd(data.dir)
RMScurrent<-fread("Orchard_RMS_ALL.csv",sep=",")
...
p.SSC <- ggplot(MG2,aes(DateTime, SSC)) + geom_point(stat="identity", colour="#FF7F00", size=0.3) + theme_classic(base_size = 12)+scale_y_continuous(position = "left")+
		...
		scale_x_datetime(expand=c(0,0))+annotate(geom="text", x=as.POSIXct("2016-06-25 12:00", format = "%Y-%m-%d %H:%M"), y=550, label= "a.", size=6)
...
png(paste(image.dir,"Orchard Rocks July 2020.png",sep=''),width=40, height=(40/3)*2, units='cm', res=500, pointsize=10, bg='white')
...
library(scales)
xpos=c(as.POSIXct("2016-06-01 12:00", format = "%Y-%m-%d %H:%M") ,as.POSIXct("2017-01-31 23:50", format = "%Y-%m-%d %H:%M"))
...
p.RMS <- ggplot(MG2, aes(DateTime, RMS)) + geom_point(stat="identity",colour="#4DAF4A", size=0.3) + theme_minimal() + theme_classic(base_size = 12)+scale_y_continuous(position = "right",)+
        ...
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),axis.title.y = element_text(colour="grey20",size=12))+ylab("RMS (pressure)")+
		scale_x_datetime(expand=c(0,0),limits =xpos)+annotate(geom="text", x=as.POSIXct("2016-06-05 12:00", format = "%Y-%m-%d %H:%M") , y=0.3, label= "b.", size=6) 
...
p.SSC <- ggplot(MG2,aes(DateTime, SSC)) + geom_point(stat="identity", colour="#FF7F00", size=0.3) + theme_classic(base_size = 12)+scale_y_continuous(position = "left")+
        ...
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),axis.title.y = element_text(colour="grey20",size=12))+ylab(bquote('SSC' ~(mg.L^-1)))+
		scale_x_datetime(expand=c(0,0),limits =xpos)+annotate(geom="text", x=as.POSIXct("2016-06-05 12:00", format = "%Y-%m-%d %H:%M"), y=175, label= "a.", size=6)+ylim(0,200)
...
p.RMScurrent <-ggplot(MG2,aes(DateTime, as.numeric(currentRMS))) + geom_point(stat="identity", colour="darkslateblue", size=0.3) + theme_classic(base_size = 12)+
		scale_y_continuous(position = "right",limits=c(0,0.005))+
		theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),axis.title.y = element_text(colour="grey20",size=12))+ylab(bquote('RMS (current)'))+
		scale_x_datetime(expand=c(0,0),limits =xpos)+annotate(geom="text", x=as.POSIXct("2016-06-05 12:00", format = "%Y-%m-%d %H:%M"), y=0.004, label= "d.", size=6)
...
#plot_grid(p.SSC,p.RMS, p.Speed, p.RMScurrent,p.Hs, p.Height,p.flow, align = "v", nrow = 7)	
	...
	png(paste(image.dir,"Orchard Rocks 2016 June-Jan.png",sep=''),width=40, height=(40/3)*2, units='cm', res=500, pointsize=10, bg='white')
        par(mar=c(4,4,1,1),cex=1,oma=c(2,0,1,0.5)) #		 
