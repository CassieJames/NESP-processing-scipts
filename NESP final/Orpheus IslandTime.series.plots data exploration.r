...
trapdata<- read_xlsx("Logger metadata_for import.xlsx", sheet="Orpheus Island")
trapdata=as.data.frame(trapdata)
trapdata=trapdata[,c(2,3,6,8)]
data_long <- gather(trapdata, Trap, Measurement, Orpheus.Island:Pelorus.Island,factor_key=TRUE)
...
p5.light <- ggplot(tdata,aes(timestamp, light)) + geom_rect(data=tdata, aes(xmin = min(timestamp), xmax = max(timestamp), ymin=0, ymax=+Inf), fill='grey')+ 
		geom_line(stat="identity", colour="white", size=0.3) + theme_classic(base_size = 12)+scale_y_continuous(limits = c(0, 520),position = "right")+
...
p3.trap <- ggplot(data_long, aes(colour = Trap, x = data_long$Date.in, xend = data_long$Date.out, y = data_long$Measurement, yend = data_long$Measurement)) +theme_classic(base_size = 12)+
	...
	theme(axis.title.x=element_blank(),
		...
		scale_color_manual(values=c("chocolate4","chocolate2"))+scale_y_continuous(limits = c(0, 25),position = "left")+
		scale_x_date(expand=c(0,0))+annotate(geom="text", x=as.Date("2016-06-25"), y=c(95/4), label= "a.", size=6) 
...
png(paste(image.dir,"Orpheus Island May 2020.png",sep=''),width=40, height=(40/3)*2, units='cm', res=500, pointsize=10, bg='white')
...
png(paste(image.dir,"Orpheus Island Tide amplitude vs current data.png",sep=''),width=20, height=10, units='cm', res=500, pointsize=10, bg='white')

par(mfrow=c(1,2))
plot(MG$u_comp_current,MG$v_comp_current,ylim=c(-0.4,0.4), xlim=c(-0.4,0.4),col='grey',ylab="V component",xlab="U component")
...
png(paste(image.dir,"Orpheus Island Tide amplitude vs current data by year.png",sep=''),width=30, height=30, units='cm', res=500, pointsize=10, bg='white')		  
...
png(paste(image.dir,"Orpheus Island Tide amplitude vs current data by month 2018.png",sep=''),width=30, height=30, units='cm', res=500, pointsize=10, bg='white')	
P4<-ggplot(MG2018,aes(u_comp_current,v_comp_current), na.rm=FALSE) + geom_point(colour="grey")+facet_wrap(MG2018$Month, nrow=2)+xlim(-0.4,0.4)+ylim(-0.4,0.4)
P4
dev.off()

MG2019<-MG[MG$Year==2019,]

png(paste(image.dir,"Orpheus Island Tide amplitude vs current data by month 2019.png",sep=''),width=30, height=30, units='cm', res=500, pointsize=10, bg='white')	
P5<-ggplot(MG2019,aes(u_comp_current,v_comp_current), na.rm=FALSE) + geom_point(colour="grey")+facet_wrap(MG2019$Month, nrow=2)+xlim(-0.4,0.4)+ylim(-0.4,0.4)
P5
dev.off()

png(paste(image.dir,"Orpheus Island Date vs current speed 2019.png",sep=''),width=30, height=30, units='cm', res=500, pointsize=10, bg='white')	
plot(MG2019$'Date', MG2019$'speed (m/s)')
dev.off()

png(paste(image.dir,"Orpheus Island Date vs current speed 2018.png",sep=''),width=30, height=30, units='cm', res=500, pointsize=10, bg='white')	
plot(MG2018$'Date', MG2018$'speed (m/s)')
dev.off()

png(paste(image.dir,"Orpheus Island Date vs current speed Full record.png",sep=''),width=30, height=30, units='cm', res=500, pointsize=10, bg='white')	
