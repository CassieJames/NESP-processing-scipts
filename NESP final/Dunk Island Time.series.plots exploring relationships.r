...
TV_tide_ht<-fread("Clump_tide_Ht_10Mins.csv",sep=",")
...
p.RMS <- ggplot(MG2, aes(DateTime, RMS)) + geom_point(stat="identity",colour="#4DAF4A", size=0.3) + theme_minimal() + theme_classic(base_size = 14)+scale_y_continuous(position = "right")+
		theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),axis.title.y = element_text(colour="grey20",size=16))+ylab("RMS (pressure)")+
		scale_x_datetime(expand=c(0,0))+annotate(geom="text", x=as.POSIXct("2016-06-25 12:00", format = "%Y-%m-%d %H:%M") , y=ypos, label= "b.", size=8) 

ypos = min(MG2$SSC,na.rm = TRUE) + 0.95*diff(range(MG2$SSC,na.rm = TRUE))
		
p.SSC <- ggplot(MG2,aes(DateTime, SSC)) + geom_point(stat="identity", colour="#FF7F00", size=0.3) + theme_classic(base_size = 14)+scale_y_continuous(position = "left")+
		theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),axis.title.y = element_text(colour="grey20",size=16))+ylab(bquote('SSC' ~(mg.L^-1)))+
		scale_x_datetime(expand=c(0,0))+annotate(geom="text", x=as.POSIXct("2016-06-25 12:00", format = "%Y-%m-%d %H:%M"), y=ypos, label= "a.", size=8) 

ypos = min(MG2$NTUe,na.rm = TRUE) + 0.95*diff(range(MG2$NTUe,na.rm = TRUE))

p.NTUe <- ggplot(MG2,aes(DateTime, NTUe)) + geom_point(stat="identity", colour="#FF7F00", size=0.3) + theme_classic(base_size = 14)+scale_y_continuous(position = "left")+
		theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),axis.title.y = element_text(colour="grey20",size=16))+ylab(bquote('NTUe'))+
		scale_x_datetime(expand=c(0,0))+annotate(geom="text", x=as.POSIXct("2016-06-25 12:00", format = "%Y-%m-%d %H:%M"), y=ypos, label= "d.", size=8) 


ypos = min(as.numeric(MG2$Hs),na.rm = TRUE) + 0.95*diff(range(as.numeric(MG2$Hs),na.rm = TRUE))		
p.Hs <- ggplot(MG2,aes(DateTime, as.numeric(Hs))) + geom_point(stat="identity", colour="steelblue", size=0.3) + theme_classic(base_size = 14)+scale_y_continuous(position = "left")+
		theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),axis.title.y = element_text(colour="grey20",size=16))+ylab(bquote('Sig wave Ht (m)'))+
		scale_x_datetime(expand=c(0,0))+annotate(geom="text", x=as.POSIXct("2016-06-25 12:00", format = "%Y-%m-%d %H:%M"), y=ypos, label= "e.", size=8) 
		
ypos = min(as.numeric(MG2$Hmax),na.rm = TRUE) + 0.95*diff(range(as.numeric(MG2$Hmax),na.rm = TRUE))		
p.Hmax <- ggplot(MG2,aes(DateTime, as.numeric(Hmax))) + geom_point(stat="identity", colour="darkslateblue", size=0.3) + theme_classic(base_size = 14)+scale_y_continuous(position = "left")+
		theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),axis.title.y = element_text(colour="grey20",size=16))+ylab(bquote('Max wave Ht (m)'))+
		scale_x_datetime(expand=c(0,0))+annotate(geom="text", x=as.POSIXct("2016-06-25 12:00", format = "%Y-%m-%d %H:%M"), y=ypos, label= "e.", size=8) 
		
ypos = min(MG2$Speed,na.rm = TRUE) + 0.95*diff(range(MG2$Speed,na.rm = TRUE))		
#ypos=c(0,0.75)
p.Speed <- ggplot(MG2,aes(DateTime, as.numeric(Speed))) + geom_point(stat="identity", colour="darkslategray3", size=0.3) + theme_classic(base_size = 14)+scale_y_continuous(position = "left")+
		theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),axis.title.y = element_text(colour="grey20",size=16))+ylab(bquote('Current (m/s)'))+
		scale_x_datetime(expand=c(0,0))+annotate(geom="text", x=as.POSIXct("2016-06-25 12:00", format = "%Y-%m-%d %H:%M"), y=0.45, label= "c.", size=8)+ylim(0,0.5)
		
		
ypos = min(MG2$Height,na.rm = TRUE) + 0.95*diff(range(MG2$Height,na.rm = TRUE))		
p.Height <- ggplot(MG2,aes(DateTime, as.numeric(Height))) + geom_point(stat="identity", colour="darkslateblue", size=0.3) + theme_classic(base_size = 14)+scale_y_continuous(position = "right")+
		theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),axis.title.y = element_text(colour="grey20",size=16))+ylab(bquote('Tide height (m)'))+
		scale_x_datetime(expand=c(0,0))+annotate(geom="text", x=as.POSIXct("2016-06-25 12:00", format = "%Y-%m-%d %H:%M"), y=ypos, label= "f.", size=8)#+ylim(0,0.8)
		
ypos = min(MG2$currentRMS,na.rm = TRUE) + 0.95*diff(range(MG2$currentRMS,na.rm = TRUE))				
p.RMScurrent <-ggplot(MG2,aes(DateTime, as.numeric(currentRMS))) + geom_point(stat="identity", colour="darkslateblue", size=0.3) + theme_classic(base_size = 14)+
		scale_y_continuous(position = "right", limits=c(0,0.01))+
		theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),axis.title.y = element_text(colour="grey20",size=16))+ylab(bquote('RMS (current)'))+
		scale_x_datetime(expand=c(0,0))+annotate(geom="text", x=as.POSIXct("2016-06-25 12:00", format = "%Y-%m-%d %H:%M"), y=0.045, label= "d.", size=8)
	

			 
ypos = min(flowdata$Tully,na.rm = TRUE) + 0.95*diff(range(flowdata$Tully,na.rm = TRUE))
xpos=c(min(tdata$timestamp), max(tdata$timestamp))
xpos=as.Date(xpos, format="%m/%d/%Y")

p.flow <- ggplot(flowdata, aes(Date, Tully)) + geom_line(colour = "#377EB8") + theme_classic(base_size = 14)+scale_y_continuous(position = "left")+geom_area(position = "identity",fill="#377EB8")+
		theme(axis.title.x=element_blank(),axis.title.y = element_text(colour="grey20",size=16), axis.text.x = element_text(face="bold",size=14))+ylab(bquote('Discharge' ~(ML.d^-1)))+
		scale_x_date(date_breaks=("3 month"),date_labels = "%d/%m/%Y",expand=c(0,0),limits=xpos)+annotate(geom="text", x=as.Date("2016-06-25"), y=ypos, label= "g.", size=8)	
	
		
png(paste(image.dir,"Dunk Island December 2020.png",sep=''),width=45, height=(40/3)*2, units='cm', res=500, pointsize=14, bg='white')
        par(mar=c(4,4,1,1),cex=1,oma=c(2,0,1,0.5)) #		 
		
plot_grid(p.NTUe,p.RMS, p.Speed, p.RMScurrent,p.Hs, p.Height,p.flow, align = "v", nrow = 7)	
dev.off()

##### plots for shorter periods
library(scales)
xpos=c(as.POSIXct("2017-06-06 12:00", format = "%Y-%m-%d %H:%M") ,as.POSIXct("2018-03-01 23:50", format = "%Y-%m-%d %H:%M"))
...
p.RMS <- ggplot(MG2, aes(DateTime, RMS)) + geom_point(stat="identity",colour="#4DAF4A", size=0.3) + theme_minimal() + theme_classic(base_size = 12)+scale_y_continuous(position = "right")+
		...
		scale_x_datetime(expand=c(0,0),limits =xpos)+annotate(geom="text", x=as.POSIXct("2016-06-25 12:00", format = "%Y-%m-%d %H:%M") , y=ypos, label= "b.", size=6) 
...
p.SSC <- ggplot(MG2,aes(DateTime, SSC)) + geom_point(stat="identity", colour="#FF7F00", size=0.3) + theme_classic(base_size = 12)+scale_y_continuous(position = "left")+
		...
		scale_x_datetime(expand=c(0,0),limits =xpos)+annotate(geom="text", x=as.POSIXct("2016-06-25 12:00", format = "%Y-%m-%d %H:%M"), y=75, label= "a.", size=6) +ylim(0,100)

ypos = min(MG2$NTUe,na.rm = TRUE) + 0.95*diff(range(MG2$NTUe,na.rm = TRUE))

p.NTUe <- ggplot(MG2,aes(DateTime, NTUe)) + geom_point(stat="identity", colour="#FF7F00", size=0.3) + theme_classic(base_size = 12)+scale_y_continuous(position = "right")+
		theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),axis.title.y = element_text(colour="grey20",size=12))+ylab(bquote('NTUe'))+
		scale_x_datetime(expand=c(0,0),limits =xpos)+annotate(geom="text", x=as.POSIXct("2016-06-25 12:00", format = "%Y-%m-%d %H:%M"), y=ypos, label= "d.", size=6) 
...
p.Hs <- ggplot(MG2,aes(DateTime, as.numeric(Hs))) + geom_point(stat="identity", colour="steelblue", size=0.3) + theme_classic(base_size = 12)+scale_y_continuous(position = "left")+
		...
		scale_x_datetime(expand=c(0,0),limits =xpos)+annotate(geom="text", x=as.POSIXct("2016-06-25 12:00", format = "%Y-%m-%d %H:%M"), y=ypos, label= "e.", size=6) 
...
p.Speed <- ggplot(MG2,aes(DateTime, as.numeric(Speed))) + geom_point(stat="identity", colour="darkslategray3", size=0.3) + theme_classic(base_size = 12)+scale_y_continuous(position = "left")+
		...
		scale_x_datetime(expand=c(0,0),limits =xpos)+annotate(geom="text", x=as.POSIXct("2016-06-25 12:00", format = "%Y-%m-%d %H:%M"), y=0.45, label= "c.", size=6)+ylim(0,0.5)
...
p.Height <- ggplot(MG2,aes(DateTime, as.numeric(Height))) + geom_point(stat="identity", colour="darkslateblue", size=0.3) + theme_classic(base_size = 12)+scale_y_continuous(position = "right")+
		...
		scale_x_datetime(expand=c(0,0),limits =xpos)+annotate(geom="text", x=as.POSIXct("2016-06-25 12:00", format = "%Y-%m-%d %H:%M"), y=ypos, label= "f.", size=6)#+ylim(0,0.8)
...
p.RMScurrent <-ggplot(MG2,aes(DateTime, as.numeric(currentRMS))) + geom_point(stat="identity", colour="darkslateblue", size=0.3) + theme_classic(base_size = 12)+
		...
		scale_x_datetime(expand=c(0,0),limits =xpos)+annotate(geom="text", x=as.POSIXct("2016-06-25 12:00", format = "%Y-%m-%d %H:%M"), y=0.0075, label= "d.", size=6)
...
p.flow <- ggplot(flowdata, aes(Date, Tully)) + geom_line(colour = "#377EB8") + theme_classic(base_size = 12)+scale_y_continuous(position = "left")+geom_area(position = "identity",fill="#377EB8")+
		theme(axis.title.x=element_blank(),axis.title.y = element_text(colour="grey20",size=12))+ylab(bquote('Discharge' ~(ML.d^-1)))+
		scale_x_date(date_breaks=("2 week"),date_labels = "%d/%m/%Y",expand=c(0,0),limits=xpos)+annotate(geom="text", x=as.Date("2016-06-25"), y=ypos, label= "g.", size=6)	
...
png(paste(image.dir,"Dunk Island July 2020 2016-2017.png",sep=''),width=40, height=(40/3)*2, units='cm', res=500, pointsize=10, bg='white')
        par(mar=c(4,4,1,1),cex=1,oma=c(2,0,1,0.5)) #		 
