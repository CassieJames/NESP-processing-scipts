...
trapdata<- read_xlsx("Logger metadata_for import.xlsx", sheet="Dunk Island")
trapdata=as.data.frame(trapdata)
trapdata=trapdata[,c(2,3,6,8)]
data_long <- gather(trapdata, Trap, Measurement, Dunk.1:Dunk.2,factor_key=TRUE)
...
p4.flow <- ggplot(flowdata, aes(Date, Tully)) + geom_line(colour = "#377EB8") + theme_classic(base_size = 12)+scale_y_continuous(position = "left")+geom_area(position = "identity",fill="#377EB8")+
...
png(paste(image.dir,"Dunk Island May 2020.png",sep=''),width=40, height=(40/3)*2, units='cm', res=500, pointsize=10, bg='white')
...
png(paste(image.dir,"Dunk Island Tide amplitude vs current data.png",sep=''),width=20, height=10, units='cm', res=500, pointsize=10, bg='white')

par(mfrow=c(1,2))
plot(MG$u_comp_current,MG$v_comp_current,ylim=c(-0.4,0.4), xlim=c(-0.4,0.4),col='grey',ylab="V component",xlab="U component",main="Dunk Island")
...
png(paste(image.dir,"Dunk Island Tide amplitude vs current data by year.png",sep=''),width=30, height=30, units='cm', res=500, pointsize=10, bg='white')		  
	  
P1<-ggplot(MG,aes(u_comp_current,v_comp_current), na.rm=FALSE) + geom_point(colour="grey")+facet_wrap(MG$Year, nrow=1)+xlim(-0.4,0.4)+ylim(-0.4,0.4)+ggtitle("Dunk Island")
...
MG2017<-MG[MG$Year==2017,]

png(paste(image.dir,"Dunk Island Tide amplitude vs current data by month 2017.png",sep=''),width=30, height=30, units='cm', res=500, pointsize=10, bg='white')	
P4<-ggplot(MG2017,aes(u_comp_current,v_comp_current), na.rm=FALSE) + geom_point(colour="grey")+facet_wrap(MG2017$Month, nrow=2)+xlim(-0.4,0.4)+ylim(-0.4,0.4)+ggtitle("Dunk Island Monthly data for 2017")
P4
dev.off()

png(paste(image.dir,"Dunk Island Date vs current speed 2017.png",sep=''),width=60, height=30, units='cm', res=500, pointsize=10, bg='white')	
par(mfrow=c(2,1))
plot(MG2017$'Date', MG2017$'speed (m/s)',main="Dunk Island Current speed 2017 ")
