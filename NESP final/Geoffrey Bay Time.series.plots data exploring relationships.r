...
deploys=deploys[which(deploys$Site=="Geoffrey"),]
tdata$deploys<-NA
...
data.dir="C:/Users/jc246980/Documents/Current projects/NESP/RMS calculations/Geoffrey/"; setwd(data.dir)
RMScurrent<-fread("Geoffrey_RMS_ALL.csv",sep=",")
...
p.Speed <- ggplot(MG2,aes(DateTime, as.numeric(Speed))) + geom_point(stat="identity", colour="darkslategray3", size=0.3) + theme_classic(base_size = 12)+scale_y_continuous(position = "left")+
		...
		scale_x_datetime(expand=c(0,0))+annotate(geom="text", x=as.POSIXct("2016-06-25 12:00", format = "%Y-%m-%d %H:%M"), y=0.2, label= "c.", size=6)+ylim(0,0.25)
...
png(paste(image.dir,"Geoffrey Bay July 2020.png",sep=''),width=40, height=(40/3)*2, units='cm', res=500, pointsize=10, bg='white')
...
library(scales)
xpos=c(as.POSIXct("2017-07-01 12:00", format = "%Y-%m-%d %H:%M") ,as.POSIXct("2017-12-31 23:50", format = "%Y-%m-%d %H:%M"))
...
p.RMS <- ggplot(MG2, aes(DateTime, RMS)) + geom_point(stat="identity",colour="#4DAF4A", size=0.3) + theme_minimal() + theme_classic(base_size = 12)+scale_y_continuous(position = "right")+
        ...
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),axis.title.y = element_text(colour="grey20",size=12))+ylab("RMS (pressure)")+
		scale_x_datetime(expand=c(0,0),limits =xpos)+annotate(geom="text", x=as.POSIXct("2017-07-05 12:00", format = "%Y-%m-%d %H:%M") , y=ypos, label= "b.", size=6) 
...
p.SSC <- ggplot(MG2,aes(DateTime, SSC)) + geom_point(stat="identity", colour="#FF7F00", size=0.3) + theme_classic(base_size = 12)+scale_y_continuous(position = "left")+
        ...
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),axis.title.y = element_text(colour="grey20",size=12))+ylab(bquote('SSC' ~(mg.L^-1)))+
		scale_x_datetime(expand=c(0,0),limits =xpos)+annotate(geom="text", x=as.POSIXct("2017-07-05 12:00", format = "%Y-%m-%d %H:%M"), y=40, label= "a.", size=6)+ylim(0,50)
...
p.NTUe <- ggplot(MG2,aes(DateTime, NTUe)) + geom_line(stat="identity", colour="#FF7F00", size=0.3) + theme_classic(base_size = 12)+scale_y_continuous(position = "right")+
        ...
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),axis.title.y = element_text(colour="grey20",size=12))+ylab(bquote('NTUe'))+
		scale_x_datetime(expand=c(0,0),limits =xpos)+annotate(geom="text", x=as.POSIXct("2017-07-05 12:00", format = "%Y-%m-%d %H:%M"), y=ypos, label= "d.", size=6) 
...
p.Hs <- ggplot(MG2,aes(DateTime, as.numeric(Hs))) + geom_point(stat="identity", colour="steelblue", size=0.3) + theme_classic(base_size = 12)+scale_y_continuous(position = "left")+
        ...
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),axis.title.y = element_text(colour="grey20",size=12))+ylab(bquote('Sig wave Ht (m)'))+
		scale_x_datetime(expand=c(0,0),limits =xpos)+annotate(geom="text", x=as.POSIXct("2017-07-05 12:00", format = "%Y-%m-%d %H:%M"), y=ypos, label= "e.", size=6) 
...
p.Speed <- ggplot(MG2,aes(DateTime, as.numeric(Speed))) + geom_point(stat="identity", colour="darkslategray3", size=0.3) + theme_classic(base_size = 12)+scale_y_continuous(position = "left")+
        ...
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),axis.title.y = element_text(colour="grey20",size=12))+ylab(bquote('Current (m/s)'))+
		scale_x_datetime(expand=c(0,0),limits =xpos)+annotate(geom="text", x=as.POSIXct("2017-07-05 12:00", format = "%Y-%m-%d %H:%M"), y=0.2, label= "c.", size=6)+ylim(0,0.25)
...
p.Height <- ggplot(MG2,aes(DateTime, as.numeric(Height))) + geom_point(stat="identity", colour="darkslateblue", size=0.3) + theme_classic(base_size = 12)+scale_y_continuous(position = "right")+
        ...
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),axis.title.y = element_text(colour="grey20",size=12))+ylab(bquote('Tide height (m)'))+
		scale_x_datetime(expand=c(0,0),limits =xpos)+annotate(geom="text", x=as.POSIXct("2017-07-05 12:00", format = "%Y-%m-%d %H:%M"), y=ypos, label= "f.", size=6)#+ylim(0,0.8)
...
p.RMScurrent <-ggplot(MG2,aes(DateTime, as.numeric(currentRMS))) + geom_point(stat="identity", colour="darkslateblue", size=0.3) + theme_classic(base_size = 12)+
        ...
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),axis.title.y = element_text(colour="grey20",size=12))+ylab(bquote('RMS (current)'))+
		scale_x_datetime(expand=c(0,0),limits =xpos)+annotate(geom="text", x=as.POSIXct("2017-07-05 12:00", format = "%Y-%m-%d %H:%M"), y=0.008, label= "d.", size=6)
...
p.flow <- ggplot(flowdata, aes(Date, Burdekin)) + geom_line(colour = "#377EB8") + theme_classic(base_size = 12)+scale_y_continuous(position = "left",limits=c(0,50000))+geom_area(position = "identity",fill="#377EB8")+
		...
		scale_x_date(date_breaks=("2 week"),date_labels = "%d/%m/%Y",expand=c(0,0),limits = xpos)+annotate(geom="text", x=as.Date("2017-07-05"), y=ypos, label= "g.", size=6)

	
	plot_grid(p.SSC,p.RMS, p.Speed, p.RMScurrent,p.Hs, p.Height,p.flow, align = "v", nrow = 7)	
	
	png(paste(image.dir,"Geoffrey Bay 2017 July-Dec.png",sep=''),width=40, height=(40/3)*2, units='cm', res=500, pointsize=10, bg='white')
        par(mar=c(4,4,1,1),cex=1,oma=c(2,0,1,0.5)) #		 
