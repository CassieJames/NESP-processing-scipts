...
trapdata<- read_xlsx("Logger metadata_for import.xlsx", sheet="Middle Reef")
...
p5.light <- ggplot(tdata,aes(timestamp, light), na.rm=FALSE) + geom_rect(data=tdata, aes(xmin = min(timestamp), xmax = max(timestamp), ymin=0, ymax=+Inf), fill='grey')+ 
		geom_line(stat="identity", colour="white", size=0.3) + theme_classic(base_size = 12)+scale_y_continuous(limits = c(0, 1200),position = "right")+
...
p3.trap <- ggplot(data_long, aes(colour = "chocolate4", x = data_long$Date.in, xend = data_long$Date.out, y = data_long$Trap, yend = data_long$Trap)) +theme_classic(base_size = 12)+
	...
	theme(axis.title.x=element_blank(),
		...
		scale_color_manual(values=c("chocolate4","chocolate2"))+scale_y_continuous(limits = c(0, 75),position = "left")+
...
png(paste(image.dir,"Middle Reef May 2020.png",sep=''),width=40, height=(40/3)*2, units='cm', res=500, pointsize=10, bg='white')
...
data.dir="C:/Users/jc246980/Documents/Current projects/NESP/Tide data/"; setwd(data.dir)
Tide_Ebb<-fread("Tide_Ebb_Flood.csv",sep=",")
Tide_Ebb[,'Timestamp'] <- as.POSIXct(Tide_Ebb$Timestamp,format = "%d/%m/%Y %H:%M")

Tide_Ebb<-setkey(Tide_Ebb, Timestamp )
MG<-setDT(MG) 
MG<-setkey(MG, timestamp)
MG <- Tide_Ebb[ MG, roll = "nearest",allow.cartesian=TRUE]
...
png(paste(image.dir,"Middle Reef Tide amplitude vs current data.png",sep=''),width=20, height=10, units='cm', res=500, pointsize=10, bg='white')

par(mfrow=c(1,2))
plot(MG$u_comp_current,MG$v_comp_current,ylim=c(-0.4,0.4), xlim=c(-0.4,0.4),col='grey',ylab="V component",xlab="U component",main="Middle Reef")
...
legend(-0.025, -0.25, legend=c("Tide amplitude >3.5m", "Tide amplitude <1m"),
       col=c("blue", "red"), lty=1, cex=0.8)
	   
dev.off()
	   
	   
png(paste(image.dir,"Middle Reef Tide amplitude vs current data by year.png",sep=''),width=30, height=30, units='cm', res=500, pointsize=10, bg='white')		  
	  
P1<-ggplot(MG,aes(u_comp_current,v_comp_current), na.rm=FALSE) + geom_point(colour="grey")+facet_wrap(MG$Year, nrow=1)+xlim(-0.4,0.4)+ylim(-0.4,0.4)+ggtitle("Middle Reef")
...
P3<-ggplot(MGNeap,aes(u_comp_current,v_comp_current), na.rm=TRUE) + geom_point(colour="red")+facet_wrap(MGNeap$Year, nrow=1)+xlim(-0.4,0.4)+ylim(-0.4,0.4)+ggtitle("Tide amplitude <1.0")
...
png(paste(image.dir,"Middle Reef Tide amplitude vs current data by month 2019.png",sep=''),width=30, height=30, units='cm', res=500, pointsize=10, bg='white')
P5<-ggplot(MG2019,aes(u_comp_current,v_comp_current), na.rm=FALSE) + geom_point(colour="grey")+facet_wrap(MG2019$Month, nrow=2)+xlim(-0.4,0.4)+ylim(-0.4,0.4)+ggtitle("Middle Reef Monthly data for 2019")	
P5
dev.off()

MG2018<-MG[MG$Year==2018,]

png(paste(image.dir,"Middle Reef Tide amplitude vs current data by month 2018.png",sep=''),width=30, height=30, units='cm', res=500, pointsize=10, bg='white')	
P5<-ggplot(MG2018,aes(u_comp_current,v_comp_current), na.rm=FALSE) + geom_point(colour="grey")+facet_wrap(MG2018$Month, nrow=2)+xlim(-0.4,0.4)+ylim(-0.4,0.4)+ggtitle("Middle Reef Monthly data for 2018")
P5
dev.off()


###### Compare Ebb and Flood currents

MG_EBB<-MG[MG$Ebb_Flood=="Ebb",]
MG_FLOOD<-MG[MG$Ebb_Flood=="Flood",]

png(paste(image.dir,"Middle Reef Tide amplitude vs current data comparing ebb and flood.png",sep=''),width=20, height=10, units='cm', res=500, pointsize=10, bg='white')

par(mfrow=c(1,2))
plot(MG$u_comp_current,MG$v_comp_current,ylim=c(-0.4,0.4), xlim=c(-0.4,0.4),col='grey',ylab="V component",xlab="U component",main="Middle Reef")
plot(MG_EBB$u_comp_current,MG_EBB$v_comp_current,ylim=c(-0.4,0.4), xlim=c(-0.4,0.4),col='blue',ylab="V component",xlab="U component")
points(MG_FLOOD$u_comp_current,MG_FLOOD$v_comp_current, col='red')
legend(-0.025, -0.25, legend=c("Tide amplitude >3.5m", "Tide amplitude <1m"),
       col=c("blue", "red"), lty=1, cex=0.8)
